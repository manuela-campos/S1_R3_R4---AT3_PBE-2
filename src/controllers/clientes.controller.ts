...
export class ClientesController {
  ...
  editar = async (req: Request, res: Response) => {
    try {
      const { idCliente, nomeCliente, email } = req.body;
      // const idCliente = Number(req.query.idCliente)
      const alterado = await this._service.editarClientes(
        nomeCliente,
        email,
        Number(idCliente),
      );
  ...
  excluir = async (req: Request, res: Response) => {
    try {
      ...
      const idCliente = Number(req.query.idCliente);
      console.log("ID recebido:", req.query.idCliente);
  ...
  selecionarIdFormatado = async (req: Request, res: Response) => {
    try {
      ...
      // Chama o novo método do Service
      const clietesFormatada =
        await this._service.selecionarIdFormatado(idCliente);
